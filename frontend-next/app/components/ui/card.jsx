...
const Card = ({ problem }) => {
    ...
    return (
        <Link href={`question/${_id}`} className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row m hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 mx-2 my-1">
            <div className="flex flex-col justify-between p-4 leading-normal">
                <h6 className="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white">{title}</h6>
                <div className="flex flex-wrap mt-0">
                    {tags.map((tag, index) => (
                        <span key={index} className="bg-blue-100 text-blue-800 text-small font-medium me-2 px-2.5 py-1.5 rounded dark:bg-blue-900 dark:text-blue-300 uppercase">{tag}</span>
                    ))}
                </div>
                <div className="flex items-center mt-4">
                    <div className="name text-2 font-medium">{name}</div>
                    <div className="asktime text-1 font-light mx-1">- asked {timeAgo}</div>
                </div>
            </div>
        </Link>
    )
